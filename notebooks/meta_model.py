...
class SequentialEnsembleRegressor(BaseEstimator, RegressorMixin):
    def __init__(self, base_estimator, n_estimators=50, sample_size=0.75, lr=0.1, random_state=None, **base_params):
        """
        Meta-modelo de ensamble secuencial para regresión.

        Parameters:
        - base_estimator: objeto scikit-learn, modelo base (regresor)
        ...
        - base_params: dict, hiperparámetros adicionales para el modelo base
        """
    ...
    def fit(self, X, y):
        ...
        for _ in range(self.n_estimators):
            model = clone(self.base_estimator).set_params(**self.base_params)
